...
namespace lightEngine
{
	namespace graphics
	{
		namespace modelLoading
		{
				...
				for (int i = 0; i < assimpMesh->mNumVertices; i++)
				{
					...
					if (hasNormals)
					{
						...
						if (hasTangents)
						{
							...
							mml::Vec3 myNormal(normal.x, normal.y, normal.z);
							mml::Vec3 myTangent(tangent.x, tangent.y, tangent.z);
							mml::Vec3 myBitangent(bitangent.x, bitangent.y, bitangent.z);
			...
			mml::Mat4 convert_assimp_matrix(const aiMatrix4x4& assimpMatrix)
			{
				mml::Mat4 ourMatrix(1.0f);
